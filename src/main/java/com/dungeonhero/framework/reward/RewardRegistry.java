...
public final class RewardRegistry {
    ...
    public synchronized RewardResult grant(GameplayContext context, GameplayDefinition definition) {
        ...
        return reward == null ? new RewardResult(false, "Unknown reward: " + definition.type())
                : reward.grant(context, definition);
    }

    public synchronized List<RewardResult> grantAll(GameplayContext context, List<GameplayDefinition> definitions) {
        return definitions == null ? List.of() : definitions.stream().map(definition -> grant(context, definition)).toList();
    }
