...
public final class ConfigurationValidator {
    ...
    public static ConfigurationValidationResult validate(FeatureConfig config) {
        ...
        if (version != null && (!(version instanceof Number) || ((Number) version).intValue() < 1)) {
            issues.add(new ConfigurationIssue(config.featureId() + ".ConfigVersion", "must be a positive number"));
        } else if (version instanceof Number && ((Number) version).intValue() > FeatureConfig.CURRENT_VERSION) {
            issues.add(new ConfigurationIssue(config.featureId() + ".ConfigVersion",
        ...
        for (String key : List.of("Objectives", "Conditions", "Actions", "Rewards", "Triggers")) {
            ...
            for (int index = 0; index < values.size(); index++) {
                ...
                if (!(definition instanceof Map<?, ?> map)) {
                    issues.add(new ConfigurationIssue(config.featureId() + "." + key + "[" + index + "]",
                            "must be a map containing type"));
                    continue;
                }
                Object type = findIgnoreCase(map, "type");
                if (!(type instanceof String value) || value.isBlank()) {
                    issues.add(new ConfigurationIssue(config.featureId() + "." + key + "[" + index + "].type",
        ...
        if (timers != null && !(timers instanceof ConfigurationSection)) {
            issues.add(new ConfigurationIssue(config.featureId() + ".Timers", "must be a map of positive seconds"));
            ...
            for (String key : timerSection.getKeys(false)) {
                ...
                if (!(value instanceof Number number) || number.longValue() < 1) {
                    issues.add(new ConfigurationIssue(config.featureId() + ".Timers." + key,
