...
public final class ActionRegistry {
  ...
  public synchronized ActionResult execute(GameplayContext context, GameplayDefinition definition) {
    ...
    return action == null
        ? new ActionResult(false, "Unknown action: " + definition.type())
        : action.execute(context, definition);
  }

  public synchronized List<ActionResult> executeAll(
      GameplayContext context, List<GameplayDefinition> definitions) {
    return definitions == null
        ? List.of()
        : definitions.stream().map(definition -> execute(context, definition)).toList();
  }
