...
public final class FeatureContext {
    ...
    public FeatureContext(JavaPlugin plugin, FeatureEventBus events, GameplayStateStore states,
                          ObjectiveRegistry objectives, ConditionRegistry conditions,
                          ActionRegistry actions, RewardRegistry rewards, TriggerRegistry triggers,
                          TimerService timers, Consumer<String> infoLogger, Consumer<String> warningLogger) {
        this.plugin = plugin;
        this.events = events;
        this.states = states;
    ...
    public FeatureEventBus events() { return events; }
    public GameplayStateStore states() { return states; }
    public ObjectiveRegistry objectives() { return objectives; }
    public ConditionRegistry conditions() { return conditions; }
    ...
    public TimerService timers() { return timers; }
    public void info(String message) { infoLogger.accept(message); }
    public void warn(String message) { warningLogger.accept(message); }

    public synchronized <T> FeatureEventBus.Subscription listen(String featureId, Class<T> eventType,
                                                                  Consumer<T> handler) {
    ...
    public synchronized void clearFeature(String featureId) {
        for (FeatureEventBus.Subscription subscription : subscriptions.getOrDefault(featureId, List.of())) {
            subscription.close();
        }
