...
public final class GameplayFramework {
    ...
    public GameplayFramework(JavaPlugin plugin) {
        context = new FeatureContext(plugin, events, states, objectives, conditions, actions, rewards, triggers,
                timers, plugin == null ? ignored -> { } : plugin.getLogger()::info,
                plugin == null ? ignored -> { } : plugin.getLogger()::warning);
        features = new FeatureRegistry(context);
    }

    public FeatureContext context() { return context; }
