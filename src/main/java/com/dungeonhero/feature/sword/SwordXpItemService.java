...
public final class SwordXpItemService {
    ...
    public void reload() {
        String materialName = plugin.getConfig().getString(
                "DungeonHero.Progression.SwordXPItem.Material", "EXPERIENCE_BOTTLE");
        ...
        displayName = plugin.getConfig().getString(
                "DungeonHero.Progression.SwordXPItem.Name", "&aHero Sword XP");
        lore = plugin.getConfig().getStringList("DungeonHero.Progression.SwordXPItem.Lore");
        configuredXp = Math.max(1, plugin.getConfig().getInt(
                "DungeonHero.Progression.SwordXPItem.XP", 25));
    }
    ...
    public int getXpAmount(ItemStack item) {
        ...
        Integer amount = item.getItemMeta().getPersistentDataContainer().get(xpKey, PersistentDataType.INTEGER);
