...
/**
 * Framework-backed composition module for the existing open-world dungeon loop. DungeonCore remains
 * the authoritative spawner; this module consumes gameplay events and composes configured
 * objectives, conditions, actions, and rewards.
 */
public final class OpenWorldDungeonFeature implements GameplayFeature, Listener {
  ...
  public void load(FeatureContext context, FeatureConfig config) {
    ...
    worlds =
        config.getStringList("Worlds").stream()
  ...
  private void processObjective(MobDefeatedEvent event, GameplayDefinition objective) {
    ...
    for (GameplayDefinition reward : rewards) {
      ...
      context
          .events()
          .publish(
              new RewardGrantedEvent(
                  ID, objective.id(), event.playerId(), reward.type(), rewardResult));
    }
    for (GameplayDefinition action : actions) {
      ActionResult actionResult = context.actions().execute(gameplayContext, action);
      if (!actionResult.success()) {
        context.warn(
            "Open-World Dungeon action '" + action.type() + "' failed: " + actionResult.message());
      }
    }
  }
